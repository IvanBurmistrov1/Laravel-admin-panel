        ...
        <div class="relative flex items-top justify-center min-h-screen dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        @if (\Illuminate\Support\Facades\Auth::user()->isDisabled())
                            <a href="{{ url('/') }}" class="text-sm text-gray-700 underline">Главная</a>
                        @elseif(\Illuminate\Support\Facades\Auth::user()->isUser())
                            <a href="{{ url('/user/index') }}" class="text-sm text-gray-700 underline">Кабинет</a>
                            <a href="{{ url('/') }}" class="text-sm text-gray-700 underline">Главная</a>
                        @elseif (\Illuminate\Support\Facades\Auth::user()->isVisitor())
                            <a href="{{ url('/') }}" class="text-sm text-gray-700 underline">Главная</a>
                        @elseif (\Illuminate\Support\Facades\Auth::user()->isAdministrator())
                            <a href="{{ url('/home') }}" class="text-sm text-gray-700 underline">Главная</a>
                            <a href="{{ url('/admin/index') }}" class="text-sm text-gray-700 underline">Панель администратора</a>
                        @endif
